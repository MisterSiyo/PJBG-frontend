...
export default function SignIn({ onSubmit }) { 
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (data.message === 'Email not recognized !') {
                    alert('Email unknown. Please verify your address or create an account.'); // Message pour un email non reconnu
    ...
    return (
        <div className={styles.authContainer}>
            ...
            <button className={styles.authButton} onClick={handleSubmit}>Enter</button>
        </div>
    );
}
